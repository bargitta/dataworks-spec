...
/**
 * A server that listens locally for connections from client launched by the library. Each client has a secret that it
 * needs to send to the server to identify itself and establish the session.
 * <p>
 * I/O is currently blocking (one thread per client). Clients have a limited time to connect back to the server,
 * otherwise the server will ignore the connection.
 * <p>
 * === Architecture Overview ===
 * <p>
 * The launcher server is used when Spark apps are launched as separate processes than the calling app. It looks more or
 * less like the following:
 * <p>
 * -----------------------                       ----------------------- |      User App       |     spark-submit      |
 * Spark App      | |                     |  -------------------> |                     | |         ------------|
 * |-------------        | |         |           |        hello          |            |        | | | L. Server
 * |<----------------------| L. Backend |        | |         |           |                       | |        | |
 * -------------                       ----------------------- |               |     | ^ |               v     |
 ...
 * App Handle |<------------------------------ |        |            | -----------------------
 * <p>
 * The server is started on demand and remains active while there are active or outstanding clients, to avoid opening
 * too many ports when multiple clients are launched. Each client is given a unique secret, and have a limited amount of
 * time to connect back ({@link SparkLauncher#CHILD_CONNECTION_TIMEOUT}), at which point the server will throw away that
 * client's state. A client is only allowed to connect back to the server once.
 * <p>
 * The launcher server listens on the localhost only, so it doesn't need access controls (aside from the per-app secret)
 * nor encryption. It thus requires that the launched app has a local process that communicates with the server. In
 * cluster mode, this means that the client that launches the application must remain alive for the duration of the
 * application (or until the app handle is disconnected).
 */
class LauncherServer implements Closeable {
    ...
    private void acceptConnections() {
        try {
            while (running) {
                ...
                if (timeoutMs > 0) {
                    scheduledExecutorService.schedule(timeout, timeoutMs, TimeUnit.MILLISECONDS);
                } else {
                    timeout.run();
                }
    ...
    class ServerConnection extends LauncherConnection {
        ...
        protected void handle(LauncherProtocol.Message msg) throws IOException {
            try {
                if (msg instanceof LauncherProtocol.Hello) {
                    ...
                    } else if (msg instanceof LauncherProtocol.SetState) {
                        handle.setState(((LauncherProtocol.SetState)msg).state);
                    } else {
        ...
        /**
         * Wait for the remote side to close the connection so that any pending data is processed. This ensures any
         * changes reported by the child application take effect.
         * <p>
         * This method allows a short period for the above to happen (same amount of time as the connection timeout,
         * which is configurable). This should be fine for well-behaved applications, where they close the connection
         * arond the same time the app handle detects the app has finished.
         * <p>
         * In case the connection is not closed within the grace period, this method forcefully closes it and any
         * subsequent data that may arrive will be ignored.
         */
