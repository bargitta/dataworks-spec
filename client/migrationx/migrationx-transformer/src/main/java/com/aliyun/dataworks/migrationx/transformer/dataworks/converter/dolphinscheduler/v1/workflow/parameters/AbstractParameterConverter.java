...
public abstract class AbstractParameterConverter<T extends AbstractParameters> {
    ...
    protected AbstractParameterConverter(Properties properties,
            SpecWorkflow specWorkflow, ProcessMeta processMeta, TaskNode taskDefinition) {
        super();
    ...
    protected SpecNode newSpecNode(TaskNode taskDefinition) {
        ...
        specNode.setRerunTimes(taskDefinition.getMaxRetryTimes());
        // Unit conversion, minutes to milliseconds
        specNode.setRerunInterval((int) Duration.ofMinutes(taskDefinition.getRetryInterval()).toMillis());

        try {
            Integer timeout = Integer.parseInt(taskDefinition.getTimeout());
            specNode.setTimeout(timeout);
        } catch (Exception e) {
            log.warn("parse task  {}, timeout {}  error: ", taskDefinition.getId(), taskDefinition.getTimeout());
        }
    ...
    private SpecTrigger convertByTaskDefinition() {
        ...
        specTrigger.setRecurrence("NORMAL".equals(taskDefinition.getRunFlag()) ? NodeRecurrenceType.NORMAL : NodeRecurrenceType.PAUSE);
    ...
    protected List<SpecVariable> convertSpecNodeParam(SpecNode specNode) {
        ParamListConverter paramListConverter = new ParamListConverter(this.parameter.getLocalParams(), taskDefinition);
